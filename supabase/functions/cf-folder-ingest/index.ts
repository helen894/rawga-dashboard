...
// ----------------------------------------------------------------------------
// 짝이 되는 Apps Script(cf-folder-ingest.gs)가 드라이브 업로드 폴더의 엑셀을
// 파싱해 { secret, action:'push', rows:[{date,desc,in,out,status}] } 형태로 POST.
// 이 함수는 그 rows 를 기존 cf_data 배열 "뒤에 append" 한다. (덮어쓰기 없음)
//   · 중복(거래일+거래내용+금액+상태 동일)은 skip
//   · 중분류(mid_cat)는 빈칸으로 적재 → 대시보드 "✨ 자동분류 추천"으로 분류
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      // 중복(거래일+거래내용+상태+금액) → skip (기존 데이터 보존)
      const dup = cfData.some((d) => {
