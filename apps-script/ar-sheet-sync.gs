...
function parseTab_(sh, cfg) {
  ...
  /* 헤더 행 찾기: expected 헤더 후보 중 하나가 들어있는 행. 상단 100행까지 훑는다.
     표 위에 제목·합계·빈 행이 쌓여 있는 탭이 많다(실측: 로가온 9행, 핀다·천대표 5행, CNA·숯 4행).
     탐색 비용은 무시할 만하고 못 찾으면 어차피 실패 처리라, 20행이던 걸 100행으로 넉넉히 뒀다.
     같은 탭에 표가 여러 개면 스캔 중 재검출로 따라간다(아래 참고).
     ⚠ 2026-09-04 정정: '숯 (확장)' 0건의 원인은 이 범위가 아니라 헤더 이름 끝의 역슬래시였다
       (그 탭 헤더는 8행이라 20행 제한으로도 찾을 수 있었다 — norm_ 주석 참고).
       헤더 범위 확대는 그 사고를 고친 게 아니고, 예방 목적으로 남긴 것이다. */
