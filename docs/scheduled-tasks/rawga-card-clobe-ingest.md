...
awga-card-clobe-ingest\SKILL.md` 이며,
...
→ 창을 넓히면 재적재로 자동 복구된다. 중복은 approval_id 로 차단되고, **금액이 달라진 건은
  갱신된다**(Edge 가 2026-08-18 부터 amended 처리). 넓게 봐도 손해가 없다.
...
⚠ '유령(우리에만)' 은 두 가지다: 수기 엑셀 업로드분(정상) 또는 **적재 후에 취소된 건**(수기로 지워야 함). 금액이 크면 후자인지 확인할 것.

## 4) 요약 남기기
- 정상: `카드 승인 N건 적재 (추가 X · 갱신 G · 중복 Y) · 합계 Z원 · 대조 통과` 한 줄
- **대조 실패(3-1)**: 누락·금액 불일치 건수와 금액을 반드시 남긴다 — 이걸 안 남기면 사고가 반복된다.
- 다음 경우엔 자세히: 검증 중단(사유) / **별칭 미매핑 카드 발견**(카드번호 명시 — `scripts/card-alias-map.json` 에 추가 필요) / 커넥터·수집 최신성 문제 / 적재 실패(HTTP 오류) / 금액 갱신 중 국내 건 포함
